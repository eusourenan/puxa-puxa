...
void	ft_swap_b(t_lista **pilha_b)
{
	...
	if (pilha_b == NULL) // Verificação de segurança
	{
		// Mensagem de erro?
		return ;
	}
	if (*pilha_b != NULL && (*pilha_b)->proximo != NULL) // Verifico se tem pelo menos 2 elementos
	{
		// Explicar essa sequência de movimentos vai ter que ser com algo visual, malz
		auxiliar = (*pilha_b)->proximo;
		(*pilha_b)->proximo = (*pilha_b)->proximo->proximo;
		auxiliar->proximo = *pilha_b;
		*pilha_b = auxiliar;		
	}
	else
	{
...
void	ft_rotacionar_b(t_lista **pilha_b)
{
	...
	auxiliar = ft_ultimo_da_lista(*pilha_b);
	if (auxiliar == *pilha_b) // Se isso for verdade, só tem um elemento em A (não dá pra fazer a troca de dois elementos se a lista só tem 1)
	{
		// Mensagem de erro?
		return ;
	}
...
void	ft_rotacao_reversa_b(t_lista **pilha_b)
{
	...
	auxiliar = ft_penultimo_da_lista(*pilha_b);
	...
	// Vai ser visual também (T.T) Só consigo explicar desenhando
	auxiliar->proximo->proximo = *pilha_b;
	*pilha_b = auxiliar->proximo;
	auxiliar->proximo = NULL;
}

/* Elemento no topo da pilha B, vai pro topo da pilha A
Exemplo: 
